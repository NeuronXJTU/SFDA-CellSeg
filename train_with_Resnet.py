...
def main():
    ...
    target_loader = data.DataLoader(target_dataset, batch_size=args.batch_size, shuffle=True, num_workers=2, pin_memory=False)

    # for idx in model_S.named_modules():
    #     print(idx)

    if args.mode == 'Target':
        for name ,child in (model_S.named_modules()):
            # print(name)
            if 'bn' in name:
                for param in child.parameters():
        ...
        optimizer_S = optim.Adam(filter(lambda p: p.requires_grad, model_S.parameters()), lr=lr_S)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if(args.mode == 'Target'):
            for i,(imagest, imagesTr, idt) in enumerate(target_loader):
                ...
                C, N, H, W = images_T.size()
                interp = nn.Upsample(size=( H, W), mode='bilinear', align_corners=True)
                pred_masks_T = interp(pred_masks_T)
                # print(pred_masks_T)
                pred_masks_T = torch.sigmoid(pred_masks_T)
                outputs_soft = torch.softmax(pred_masks_T, dim=1) 
                
                _, s_tgt, _ = torch.svd(outputs_soft)
                ...
                pred_masks_T_rotate = interp(pred_masks_T_rotate)
                pred_masks_T_rotate = torch.sigmoid(pred_masks_T_rotate)
                ...
                loss_target = loss_Entorpy + weight_crf * out_gatedcrf +  loss_rotate_T + alpha * BNM_method_loss
                # else:
                #     loss_target = loss_Entorpy + weight_crf * out_gatedcrf + loss_rotate_T
        ...
        assd_temp = 0
        hd95_temp = 0
        if(args.mode=='Target' or args.mode == 'Test'):
            with torch.no_grad():
                for i,(images,mask,name) in enumerate(test_vali_target_loader):
                    ...
                    output = model_S(img)
                    C, N, H, W = mask.size()
                    interp_out = nn.Upsample(size=( H, W), mode='bilinear', align_corners=True)
                    output = interp_out(output)
                    ...
                    result_t = tf2(result)
                    TensorboardWriter.add_image(str(name)+'mask', result_t, global_step=epoch, walltime=None, dataformats='CHW')
        ...
        if(args.mode=='Source'):
            with torch.no_grad():
                for i,(images,mask,name) in enumerate(test_vali_source_loader):
                    ...
                    mask = np.reshape(mask, result.shape)
                    # if epoch>5:
                    #     assd = binary.assd(result, mask, connectivity=1)
                    #     hd95 = binary.hd95(result, mask)
                    #     hd95_temp = hd95_temp + hd95
                    #     assd_temp = assd_temp + assd
                    temp = temp + miou
                    num = num + 1
